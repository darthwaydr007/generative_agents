...
ALLOWED_DEVICES ={"furnace", "shower", "tv", "piano", "kitchen sink", "refrigerator", "coffee machine", "bathroom sink", "bed"}
    ...
    try:
        ...
        if device == "furnace":
            ...
            if "ready" in description:
                status = "started"
            elif "use" or "functioning" in description:
                status = "brewing"
            else:
                status = "idle"
...
for i in range(len(rows)):
    ...
    final_rows.append({"timestamp": current_timestamp+"||"+str(i), **device_states})
...
json_to_csv(final_rows, output_csv_file)
